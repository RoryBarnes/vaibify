...
def test_happy_path_no_filter_returns_hashes_for_all_files(clientSandbox):
    ...
    with patch("requests.get", side_effect=listResponses):
...
def test_filter_miss_records_none_for_absent_keys(clientSandbox):
    ...
    with patch("requests.get",
               return_value=_fmockJsonResponse(200, dictRecord)) as mockGet:
        dictHashes = fdictFetchRemoteHashes(
            "12345",
...
def test_large_file_streams_in_chunks_without_full_load(clientSandbox):
    ...
    with patch("requests.get", side_effect=[
        _fmockJsonResponse(200, dictRecord), mockStream,
    ]):
        dictHashes = fdictFetchRemoteHashes(
            "12345", clientZenodo=clientSandbox,
        )
...
def test_iteration_order_matches_deposit_listing_order(clientSandbox):
    ...
    with patch("requests.get", side_effect=[
        _fmockJsonResponse(200, dictRecord), *listResponses,
    ]):
        dictHashes = fdictFetchRemoteHashes(
            "12345", clientZenodo=clientSandbox,
        )
...
def test_zenodo_request_includes_timeout(clientSandbox):
    ...
    assert mockGet.call_args.kwargs.get("timeout") is not None
...
def test_per_file_404_with_token_in_body_is_redacted(clientSandbox):
    ...
    with patch("requests.get", side_effect=[
        _fmockJsonResponse(200, dictRecord), mockFileFetch,
    ]):
        with pytest.raises(ZenodoNotFoundError) as excInfo:
            fdictFetchRemoteHashes(
                "12345", clientZenodo=clientSandbox,
