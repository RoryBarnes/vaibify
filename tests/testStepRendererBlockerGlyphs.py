...
_SCRIPT_UTILITIES_REL = "vaibify/gui/static/scriptUtilities.js"
...
def testNoXStatusGlyphInLevelCellRenderers():
    ...
    appear in any renderer's source, including the shared builder."""
    for sRelativePath in (_STEP_RENDERER_REL, _LEGEND_PANEL_REL,
                          _SCRIPT_UTILITIES_REL):
...
def testRendererStampsEveryLevelCellStateClass():
    """The single shared level-cell builder derives the cell class
    from the wire state (``level-cell-<sState>``) and renders the
    favicon only for the attained state; the step renderer must
    delegate to it rather than hand-writing cell markup, so the two
    can never diverge (the pre-extraction bug class)."""
    sUtilities = _fsReadText(_SCRIPT_UTILITIES_REL)
    assert "level-cell-' + fnEscapeHtml(sState)" in " ".join(
        sUtilities.split()
    ), (
        "cell class must derive from the wire sState, escaped for "
        "the attribute context"
    )
    assert re.search(
        r'sState === "attained"[\s\S]{0,200}?fsBuildAttainedFavicon',
        sUtilities,
    ), "attained cells must render the favicon image"
    assert re.search(
        r"function fsBuildAttainedFavicon[\s\S]{0,300}?favicon\.png",
        sUtilities,
    ), "the favicon builder must emit the favicon image"
    sRenderer = _fsReadText(_STEP_RENDERER_REL)
    assert "fsBuildLevelCell" in sRenderer, (
        "the step renderer must delegate to the shared builder"
    )
...
def testLegendDescribesLevelCellVocabulary():
    ...
    severities, and the favicon. Samples are generated by the shared
    ``fsBuildLevelCell`` builder — identical to live cells by
    construction — so the legend source must call the builder with
    every state, and the markup literals must live in the builder."""
    ...
    for sNeedle in ("fsBuildAttainedFavicon",
                    "regression-warning-red",
                    "regression-warning-orange"):
        if sNeedle not in sLegend:
            listMissing.append(sNeedle)
    assert listMissing == [], (
