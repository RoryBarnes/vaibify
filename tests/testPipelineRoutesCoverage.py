...
# ── Container activity sampling (out-of-band compute indicator) ──


def test_activity_sample_parses_loadavg_and_busy_count():
    """A well-formed sample yields load + busy count minus the ps."""
    from vaibify.gui.routes.pipelineRoutes import (
        _fdictParseActivitySample,
    )
    dictSample = _fdictParseActivitySample(
        "3.42 1.80 4.05 2/491 12345\n3\n",
    )
    assert dictSample == {
        "fLoadOneMinute": 3.42,
        "iBusyProcesses": 2,
...
def test_activity_sample_clamps_busy_count_at_zero():
    """The sampling ps counts itself; an idle container reads zero."""
    from vaibify.gui.routes.pipelineRoutes import (
        _fdictParseActivitySample,
    )
    dictSample = _fdictParseActivitySample(
        "0.01 0.05 0.00 1/200 999\n1\n",
    )
    assert dictSample["iBusyProcesses"] == 0


def test_activity_sample_returns_none_on_garbage():
    """Unparseable exec output must read unknown, never idle."""
    from vaibify.gui.routes.pipelineRoutes import (
        _fdictParseActivitySample,
    )
    assert _fdictParseActivitySample("") is None
    assert _fdictParseActivitySample("garbage") is None
    assert _fdictParseActivitySample("not floats\nalso not\n") is None


def test_activity_sampler_returns_none_on_exec_failure():
    """A failed docker exec must not fabricate an activity reading."""
    from unittest.mock import MagicMock
    from vaibify.gui.routes.pipelineRoutes import (
        _fdictSampleContainerActivity,
    )
    mockDocker = MagicMock()
    mockDocker.ftResultExecuteCommand.return_value = (1, "boom")
    assert _fdictSampleContainerActivity(
        {"docker": mockDocker}, "cid",
    ) is None
    mockDocker.ftResultExecuteCommand.side_effect = RuntimeError("gone")
    assert _fdictSampleContainerActivity(
        {"docker": mockDocker}, "cid",
    ) is None
