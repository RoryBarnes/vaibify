...
nothing more (researcher-reported):
...
S_DOCKER_DETAIL = (
    ...
    "rootless or Docker Desktop context). Compare the endpoint vaibify "
    "used, named below, with your shell's; if they agree, the daemon "
    "is not running. Endpoint vaibify used: unix:///var/run/docker.sock "
    "(from the active Docker context). Try: docker context ls "
    "(cause: Error while fetching server API "
    "version: ('Connection aborted.', FileNotFoundError(2, 'No such "
    "file or directory')))"
)


def _fnShowErrorToast(pageDashboard, sMessage):
    """Render one error toast the way production does.

    Through the SANITIZER and then the renderer, in that order. Calling
    ``fnShowToast`` with the raw text would bypass the very function
    that did the truncating, and the assertions below would then pass
    against a restored 200-character cut -- which is exactly what
    ...
    pageDashboard.evaluate(
        """(sText) => {
            ...
            VaibifyApp.fnShowToast(
                VaibifyUtilities.fsSanitizeErrorForUser(sText), "error");
        }""",
        sMessage,
    )
...
def test_a_long_error_toast_still_carries_its_remedy(
    ...
    _fnShowErrorToast(pageDashboard, S_DOCKER_DETAIL)
