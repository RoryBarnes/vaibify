...
terminal (researcher-reported, on a first build after a
Docker context fix).
...
@pytest.mark.falsification
def test_a_dropped_acquire_request_attaches_rather_than_reporting_failure(
    pageDashboard, serverHub,
):
    """The pinned-image path crosses the same boundary the same way.

    ``fnAcquireImage`` is the twin of ``fnBuildContainer``: the same
    modal, the same progress record, the same POST held open while a
    multi-gigabyte pull runs. Fixing only the build arm would leave the
    identical defect one function away, waiting for the next researcher
    who opens a published project rather than building their own.
    ...
    pageDashboard.evaluate(
        """(sName) => {
            document.getElementById("toastContainer").innerHTML = "";
            return VaibifyContainerManager.fnAcquireImage(sName, false);
        }""",
        S_CONTAINER_NAME,
    )
    sJoined = " | ".join(_flistToastTexts(pageDashboard))
    assert "still being obtained" in sJoined, (
        "a dropped request must say the pull survived it: " + sJoined
    )
    assert "Cannot reach Vaibify server" not in sJoined, (
        "a healthy server was reported unreachable: " + sJoined
    )
