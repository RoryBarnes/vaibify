...
in-container agent lane is authorized by the shared token alone, but only
for a container that already has an owner record (per-container scoping).
The final pair of tests proves both WebSocket route modules delegate to
the single guard rather than inlining a gate of their own.
"""
...
def _fdictOwnersWithOwner(sLeaseId=S_OWNING_LEASE):
...
def _fconnAgent(sToken=S_SHARED_TOKEN):
    """Return an agent connection: token header, no loopback origin."""
    return _FakeConnection(
...
def test_non_loopback_origin_cannot_skip_lease_via_agent_lane():
    # Even with the shared token, a non-loopback browser-style origin is
    # treated as an agent: it succeeds ONLY because the container is owned
    # and the token is valid. A real browser always carries a loopback
    # Origin, so it can never divert into this lane to skip its lease.
    dictCtx = _fdictContext(_fdictOwnersWithOwner())
    conn = _fconnBrowser(sOrigin="http://evil.example.com")
    assert webSocketAuthorization.fiContainerSessionRejectionCode(
        conn, dictCtx, S_CONTAINER,
    ) == 0
...
def test_agent_token_authorizes_owned_container():
    dictCtx = _fdictContext(_fdictOwnersWithOwner())
    conn = _fconnAgent()
    assert webSocketAuthorization.fbCheckAgentToken(
        conn, S_SHARED_TOKEN, dictCtx["dictContainerOwners"], S_CONTAINER,
    ) is True
    assert webSocketAuthorization.fiContainerSessionRejectionCode(
        conn, dictCtx, S_CONTAINER,
    ) == 0
...
def test_agent_lane_is_scoped_to_owned_containers():
    dictCtx = _fdictContext(_fdictOwnersWithOwner())
    conn = _fconnAgent()
    assert webSocketAuthorization.fbCheckAgentToken(
        conn, S_SHARED_TOKEN, dictCtx["dictContainerOwners"],
        "unowned-container",
    ) is False
    # With no loopback origin and no lease, the agent on an unowned
    # container falls through to the browser lane and is refused.
    assert webSocketAuthorization.fiContainerSessionRejectionCode(
        conn, dictCtx, "unowned-container",
    ) == 4003


def test_agent_lane_requires_the_shared_token():
    dictCtx = _fdictContext(_fdictOwnersWithOwner())
    conn = _fconnAgent(sToken="not-the-shared-token")
    assert webSocketAuthorization.fbCheckAgentToken(
        conn, S_SHARED_TOKEN, dictCtx["dictContainerOwners"], S_CONTAINER,
    ) is False
