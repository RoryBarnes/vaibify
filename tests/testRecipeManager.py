"""Tests for vaibcask.gui.recipeManager step CRUD and references."""

import pytest

from vaibcask.gui.recipeManager import (
    fbValidateRecipe,
    fsResolveVariables,
    fdictCreateStep,
    fnInsertStep,
    ...
    flistFindRecipesInContainer,
    DEFAULT_SEARCH_ROOT,
)


def _fdictBuildMinimalRecipe(iStepCount=2):
    """Return a valid recipe dict with iStepCount simple steps."""
    listSteps = []
...
def test_fbValidateRecipe_valid():
    dictRecipe = _fdictBuildMinimalRecipe()
    assert fbValidateRecipe(dictRecipe) is True


def test_fbValidateRecipe_missing_keys():
    dictMissingPlotDir = {"listSteps": []}
    assert fbValidateRecipe(dictMissingPlotDir) is False

    dictMissingSteps = {"sPlotDirectory": "Plot"}
    assert fbValidateRecipe(dictMissingSteps) is False
    ...
    assert fbValidateRecipe(dictMissingStepField) is False
...
def test_fnInsertStep_renumbers_references():
    ...
    fnInsertStep(dictRecipe, 1, dictNewStep)

    assert len(dictRecipe["listSteps"]) == 3
    assert dictRecipe["listSteps"][1]["sName"] == "Inserted"

    sUpdatedCommand = dictRecipe["listSteps"][2]["saCommands"][0]
...
def test_fnDeleteStep_renumbers_references():
    ...
    fnDeleteStep(dictRecipe, 0)

    assert len(dictRecipe["listSteps"]) == 2
    assert dictRecipe["listSteps"][0]["sName"] == "Step 2"

    sUpdatedCommand = dictRecipe["listSteps"][1]["saCommands"][0]
    assert "Step01" in sUpdatedCommand
...
def test_fnReorderStep_updates_references():
    ...
    fnReorderStep(dictRecipe, 0, 2)

    assert dictRecipe["listSteps"][2]["sName"] == "A"
    assert dictRecipe["listSteps"][0]["sName"] == "B"
...
def test_flistValidateReferences_detects_broken_refs():
    ...
    listWarnings = flistValidateReferences(dictRecipe)
...
def test_flistValidateReferences_clean_recipe():
    dictRecipe = _fdictBuildMinimalRecipe(iStepCount=1)
...
def test_flistFindRecipesInContainer_uses_search_root():
    """Verify that flistFindRecipesInContainer defaults to DEFAULT_SEARCH_ROOT."""
    ...
    class MockDockerConnection:
        ...
        def ftResultExecuteCommand(self, sContainerId, sCommand):
            ...
            return (0, "/workspace/project/recipe.json\n")
    ...
    assert "/workspace/project/recipe.json" in listPaths


def test_flistFindRecipesInContainer_custom_search_root():
    class MockDockerConnection:
        def __init__(self):
            self.sCapturedCommand = None
