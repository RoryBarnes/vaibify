...
def _fLoadMatlabValue(sFullPath, dictAccess):
    ...
    if sAggregate:
        return float(getattr(daData, sAggregate)())
    listIndices = dictAccess.get("listIndices", [0])
    iIdx = listIndices[0] if listIndices else 0
    return float(daData[iIdx])
...
def _fLoadParquetValue(sFullPath, dictAccess):
    ...
    sAggregate = dictAccess.get("sAggregate")
    if sAggregate:
        return float(getattr(np, sAggregate)(daValues))
    listIndices = dictAccess.get("listIndices", [-1])
    iIdx = listIndices[0] if listIndices else -1
    return float(daValues[iIdx])
...
def _fLoadImageValue(sFullPath, dictAccess):
    ...
    sAggregate = dictAccess.get("sAggregate")
    if sAggregate:
        return float(getattr(daPixels, sAggregate)())
    listIndices = dictAccess.get("listIndices", [0])
    return float(daPixels.flatten()[listIndices[0]])
...
def _fLoadFortranValue(sFullPath, dictAccess):
    ...
    if sAggregate:
        return float(getattr(daData, sAggregate)())
    listIndices = dictAccess.get("listIndices", [0])
    return float(daData[listIndices[0]])
...
def _fLoadSpssValue(sFullPath, dictAccess):
    ...
    return _fExtractDataframeValue(dfData, dictAccess)


def _fExtractDataframeValue(dfData, dictAccess):
...
def _fLoadCgnsValue(sFullPath, dictAccess):
    ...
    sAggregate = dictAccess.get("sAggregate")
    if sAggregate:
        return float(getattr(daData.flatten(), sAggregate)())
    listIndices = dictAccess.get("listIndices", [-1])
    return float(daData.flatten()[tuple(listIndices)])
...
def _fsGenerateIntegrityCode(listdictReports):
    ...
    listParts = [
        ...
        "import os\n",
        "import numpy as np\nimport pytest\n\n",
        f'_STEP_DIRECTORY = os.path.dirname('
...
def _fnAddExistsTest(listParts, sSafe, sFile):
    """Append a file-exists test function to listParts."""
    listParts.append(
        f"\ndef test_{sSafe}_exists():\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    assert os.path.isfile(sPath)\n"
        f"    assert os.path.getsize(sPath) > 0\n\n"
    )


def _fnAddLoadableTest(listParts, sSafe, sFile, dictReport):
    """Append a format-specific loadable test function to listParts."""
    sFormat = dictReport.get("sFormat", "")
    tShape = dictReport.get("tShape")
    dictGenerators = {
        "npy": _fsLoadableNpy,
        "npz": _fsLoadableNpz,
        "json": _fsLoadableJson,
        "csv": _fsLoadableCsv,
        "hdf5": _fsLoadableHdf5,
        "whitespace": _fsLoadableWhitespace,
        "keyvalue": _fsLoadableKeyvalue,
        "jsonl": _fsLoadableJsonl,
        "excel": _fsLoadableExcel,
        "fits": _fsLoadableFits,
        "matlab": _fsLoadableMatlab,
        "parquet": _fsLoadableParquet,
        "image": _fsLoadableImage,
        "fasta": _fsLoadableFasta,
        "fastq": _fsLoadableFastq,
        "vcf": _fsLoadableVcf,
        "bed": _fsLoadableBed,
        "gff": _fsLoadableGff,
        "sam": _fsLoadableSam,
        "syslog": _fsLoadableSyslog,
        "cef": _fsLoadableCef,
        "fixedwidth": _fsLoadableFixedwidth,
        "multitable": _fsLoadableMultitable,
        "bam": _fsLoadableBam,
        "fortran": _fsLoadableFortran,
        "spss": _fsLoadableSpss,
        "stata": _fsLoadableStata,
        "sas": _fsLoadableSas,
        "rdata": _fsLoadableRdata,
        "votable": _fsLoadableVotable,
        "ipac": _fsLoadableIpac,
        "pcap": _fsLoadablePcap,
        "vtk": _fsLoadableVtk,
        "cgns": _fsLoadableCgns,
        "safetensors": _fsLoadableSafetensors,
        "tfrecord": _fsLoadableTfrecord,
    }
    fsGenerator = dictGenerators.get(sFormat)
    if fsGenerator:
        listParts.append(fsGenerator(sSafe, sFile, dictReport))


def _fsLoadableNpy(sSafe, sFile, dictReport):
    """Return loadable test code for a numpy .npy file."""
    tShape = dictReport.get("tShape")
    sShape = f"    assert list(d.shape) == {tShape}\n" if tShape else ""
    return (
        f"\ndef test_{sSafe}_loadable():\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    d = np.load(sPath, allow_pickle=False)\n{sShape}\n"
    )


def _fsLoadableNpz(sSafe, sFile, dictReport):
    """Return loadable test code for a numpy .npz archive."""
    listKeys = dictReport.get("listColumnNames", [])
    sKeyAsserts = ""
    for sKey in listKeys:
        sKeyAsserts += f"    assert {sKey!r} in a.files\n"
    return (
        f"\ndef test_{sSafe}_loadable():\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    a = np.load(sPath, allow_pickle=False)\n"
        f"    assert len(a.files) > 0\n{sKeyAsserts}\n"
    )


def _fsLoadableJson(sSafe, sFile, dictReport):
    """Return loadable test code for a JSON file."""
    listKeys = dictReport.get("listJsonTopKeys", [])
    sKeyAsserts = ""
    for sKey in listKeys:
        sKeyAsserts += f"    assert {sKey!r} in d\n"
    return (
        f"\ndef test_{sSafe}_loadable():\n"
        f"    import json\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
...
def _fsLoadableCsv(sSafe, sFile, dictReport):
    """Return loadable test code for a CSV file."""
    tShape = dictReport.get("tShape")
    sRowAssert = ""
    if tShape:
        sRowAssert = f"    assert len(rows) == {tShape[0]}\n"
    return (
        f"\ndef test_{sSafe}_loadable():\n"
        f"    import csv\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    with open(sPath, newline='', encoding='utf-8') as fh:\n"
        f"        rows = list(csv.DictReader(fh))\n{sRowAssert}\n"
    )


def _fsLoadableHdf5(sSafe, sFile, dictReport):
    """Return loadable test code for an HDF5 file."""
    return (
        f"\ndef test_{sSafe}_loadable():\n"
        f"    import h5py\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    with h5py.File(sPath, 'r') as fh:\n"
        f"        assert len(fh.keys()) > 0\n\n"
    )


def _fsLoadableWhitespace(sSafe, sFile, dictReport):
    """Return loadable test code for a whitespace-delimited file."""
    tShape = dictReport.get("tShape")
    sRowAssert = ""
    if tShape:
        sRowAssert = f"    assert len(rows) >= {tShape[0]}\n"
    return (
        ...
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    with open(sPath, encoding='utf-8') as fh:\n"
        f"        rows = [l for l in fh if l.strip() "
        f"and not l.strip().startswith('#')]\n"
        f"{sRowAssert}\n"
    )


def _fsLoadableKeyvalue(sSafe, sFile, dictReport):
    """Return loadable test code for a key=value text file."""
    return (
        f"\ndef test_{sSafe}_loadable():\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    with open(sPath, encoding='utf-8') as fh:\n"
        f"        lines = [l for l in fh if '=' in l]\n"
        f"    assert len(lines) > 0\n\n"
    )


def _fsLoadableJsonl(sSafe, sFile, dictReport):
    """Return loadable test code for a JSONL file."""
    return (
        f"\ndef test_{sSafe}_loadable():\n"
        f"    import json\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    with open(sPath, encoding='utf-8') as fh:\n"
        f"        records = [json.loads(l) for l in fh if l.strip()]\n"
        f"    assert len(records) > 0\n\n"
    )


def _fsLoadableExcel(sSafe, sFile, dictReport):
    """Return loadable test code for an Excel file."""
    return (
        f"\ndef test_{sSafe}_loadable():\n"
        f"    import openpyxl\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    wb = openpyxl.load_workbook(sPath, read_only=True)\n"
        f"    ws = wb.active\n"
        f"    assert ws.max_row > 0\n"
        f"    wb.close()\n\n"
    )


def _fsLoadableFits(sSafe, sFile, dictReport):
    """Return loadable test code for a FITS file."""
    return (
        f"\ndef test_{sSafe}_loadable():\n"
        f"    from astropy.io import fits\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    with fits.open(sPath) as hduList:\n"
        f"        assert len(hduList) > 0\n\n"
    )


def _fsLoadableMatlab(sSafe, sFile, dictReport):
    """Return loadable test code for a MATLAB .mat file."""
    return (
        f"\ndef test_{sSafe}_loadable():\n"
        f"    from scipy.io import loadmat\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    d = loadmat(sPath)\n"
        f"    keys = [k for k in d if not k.startswith('__')]\n"
        f"    assert len(keys) > 0\n\n"
    )


def _fsLoadableParquet(sSafe, sFile, dictReport):
    """Return loadable test code for a Parquet file."""
    return (
        f"\ndef test_{sSafe}_loadable():\n"
        f"    import pyarrow.parquet as pq\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    table = pq.read_table(sPath)\n"
        f"    assert table.num_rows > 0\n\n"
    )


def _fsLoadableImage(sSafe, sFile, dictReport):
    """Return loadable test code for an image file."""
    return (
        f"\ndef test_{sSafe}_loadable():\n"
        f"    from PIL import Image\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    img = Image.open(sPath)\n"
        f"    assert img.size[0] > 0 and img.size[1] > 0\n\n"
    )


def _fsLoadableFasta(sSafe, sFile, dictReport):
    """Return loadable test code for a FASTA file."""
    return (
        f"\ndef test_{sSafe}_loadable():\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    with open(sPath, encoding='utf-8') as fh:\n"
        f"        lines = fh.readlines()\n"
        f"    headers = [l for l in lines if l.startswith('>')]\n"
        f"    assert len(headers) > 0\n\n"
    )


def _fsLoadableFastq(sSafe, sFile, dictReport):
    """Return loadable test code for a FASTQ file."""
    return (
        f"\ndef test_{sSafe}_loadable():\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    with open(sPath, encoding='utf-8') as fh:\n"
        f"        lines = fh.readlines()\n"
        f"    assert len(lines) >= 4\n"
        f"    assert lines[0].startswith('@')\n\n"
    )


def _fsLoadableVcf(sSafe, sFile, dictReport):
    """Return loadable test code for a VCF file."""
    return (
        f"\ndef test_{sSafe}_loadable():\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    with open(sPath, encoding='utf-8') as fh:\n"
        f"        lines = fh.readlines()\n"
        f"    header = [l for l in lines if l.startswith('#CHROM')]\n"
        f"    assert len(header) > 0\n\n"
    )


def _fsLoadableBed(sSafe, sFile, dictReport):
    """Return loadable test code for a BED file."""
    return (
        f"\ndef test_{sSafe}_loadable():\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    with open(sPath, encoding='utf-8') as fh:\n"
        f"        lines = [l for l in fh if l.strip()]\n"
        f"    assert len(lines) > 0\n"
        f"    assert len(lines[0].split('\\t')) >= 3\n\n"
    )


def _fsLoadableGff(sSafe, sFile, dictReport):
    """Return loadable test code for a GFF/GTF file."""
    return (
        f"\ndef test_{sSafe}_loadable():\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    with open(sPath, encoding='utf-8') as fh:\n"
        f"        lines = [l for l in fh if not l.startswith('#')]\n"
        f"    assert len(lines) > 0\n"
        f"    assert len(lines[0].split('\\t')) >= 9\n\n"
    )


def _fsLoadableSam(sSafe, sFile, dictReport):
    """Return loadable test code for a SAM file."""
    return (
        f"\ndef test_{sSafe}_loadable():\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    with open(sPath, encoding='utf-8') as fh:\n"
        f"        lines = [l for l in fh if not l.startswith('@')]\n"
        f"    assert len(lines) > 0\n"
        f"    assert len(lines[0].split('\\t')) >= 11\n\n"
    )


def _fsLoadableSyslog(sSafe, sFile, dictReport):
    """Return loadable test code for a syslog file."""
    return (
        f"\ndef test_{sSafe}_loadable():\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    with open(sPath, encoding='utf-8') as fh:\n"
...
def _fsLoadableCef(sSafe, sFile, dictReport):
    """Return loadable test code for a CEF file."""
    return (
        f"\ndef test_{sSafe}_loadable():\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    with open(sPath, encoding='utf-8') as fh:\n"
        f"        lines = [l for l in fh if l.strip()]\n"
        f"    assert any(l.startswith('CEF:') for l in lines)\n\n"
    )


def _fsLoadableFixedwidth(sSafe, sFile, dictReport):
    """Return loadable test code for a fixed-width file."""
    return (
        f"\ndef test_{sSafe}_loadable():\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    with open(sPath, encoding='utf-8') as fh:\n"
...
def _fsLoadableMultitable(sSafe, sFile, dictReport):
    """Return loadable test code for a multi-table file."""
    return (
        f"\ndef test_{sSafe}_loadable():\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    with open(sPath, encoding='utf-8') as fh:\n"
        f"        content = fh.read()\n"
        f"    assert len(content.strip()) > 0\n\n"
    )


def _fsLoadableBam(sSafe, sFile, dictReport):
    """Return loadable test code for a BAM file."""
    return (
        f"\ndef test_{sSafe}_loadable():\n"
        f"    import pysam\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    sf = pysam.AlignmentFile(sPath, 'rb')\n"
        f"    assert sf.mapped + sf.unmapped > 0\n"
        f"    sf.close()\n\n"
    )


def _fsLoadableFortran(sSafe, sFile, dictReport):
    """Return loadable test code for a FORTRAN binary file."""
    return (
        f"\ndef test_{sSafe}_loadable():\n"
        f"    from scipy.io import FortranFile\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    ff = FortranFile(sPath, 'r')\n"
        f"    d = ff.read_reals()\n"
        f"    ff.close()\n"
        f"    assert len(d) > 0\n\n"
    )
...
def _fsLoadableSpss(sSafe, sFile, dictReport):
    """Return loadable test code for an SPSS file."""
    return (
        f"\ndef test_{sSafe}_loadable():\n"
        f"    import pyreadstat\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    df, _ = pyreadstat.read_sav(sPath)\n"
        f"    assert len(df) > 0\n\n"
    )


def _fsLoadableStata(sSafe, sFile, dictReport):
    """Return loadable test code for a Stata file."""
    return (
        f"\ndef test_{sSafe}_loadable():\n"
        f"    import pyreadstat\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    df, _ = pyreadstat.read_dta(sPath)\n"
        f"    assert len(df) > 0\n\n"
    )


def _fsLoadableSas(sSafe, sFile, dictReport):
    """Return loadable test code for a SAS file."""
    return (
        f"\ndef test_{sSafe}_loadable():\n"
        f"    import pyreadstat\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    df, _ = pyreadstat.read_sas7bdat(sPath)\n"
        f"    assert len(df) > 0\n\n"
    )


def _fsLoadableRdata(sSafe, sFile, dictReport):
    """Return loadable test code for an R data file."""
    return (
        f"\ndef test_{sSafe}_loadable():\n"
        f"    import pyreadr\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    d = pyreadr.read_r(sPath)\n"
        f"    assert len(d) > 0\n\n"
    )


def _fsLoadableVotable(sSafe, sFile, dictReport):
    """Return loadable test code for a VOTable file."""
    return (
        f"\ndef test_{sSafe}_loadable():\n"
        f"    from astropy.io.votable import parse\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    vot = parse(sPath)\n"
        f"    table = vot.get_first_table().to_table()\n"
        f"    assert len(table) > 0\n\n"
    )


def _fsLoadableIpac(sSafe, sFile, dictReport):
    """Return loadable test code for an IPAC table file."""
    return (
        f"\ndef test_{sSafe}_loadable():\n"
        f"    from astropy.io import ascii\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    table = ascii.read(sPath, format='ipac')\n"
        f"    assert len(table) > 0\n\n"
    )


def _fsLoadablePcap(sSafe, sFile, dictReport):
    """Return loadable test code for a PCAP file."""
    return (
        f"\ndef test_{sSafe}_loadable():\n"
        f"    from scapy.all import rdpcap\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    packets = rdpcap(sPath)\n"
        f"    assert len(packets) > 0\n\n"
    )


def _fsLoadableVtk(sSafe, sFile, dictReport):
    """Return loadable test code for a VTK file."""
    return (
        f"\ndef test_{sSafe}_loadable():\n"
        f"    import pyvista\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    mesh = pyvista.read(sPath)\n"
        f"    assert mesh.n_points > 0\n\n"
    )


def _fsLoadableCgns(sSafe, sFile, dictReport):
    """Return loadable test code for a CGNS file."""
    return (
        f"\ndef test_{sSafe}_loadable():\n"
        f"    import h5py\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
...
def _fsLoadableSafetensors(sSafe, sFile, dictReport):
    """Return loadable test code for a safetensors file."""
    return (
        f"\ndef test_{sSafe}_loadable():\n"
        f"    from safetensors import safe_open\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    with safe_open(sPath, framework='numpy') as fh:\n"
        f"        assert len(list(fh.keys())) > 0\n\n"
    )


def _fsLoadableTfrecord(sSafe, sFile, dictReport):
    """Return loadable test code for a TFRecord file."""
    return (
        f"\ndef test_{sSafe}_loadable():\n"
        f"    from tfrecord.reader import tfrecord_iterator\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    records = list(tfrecord_iterator(sPath))\n"
        f"    assert len(records) > 0\n\n"
    )


def _fnAddNoNanTest(listParts, sSafe, sFile, dictReport):
    """Append a no-NaN/Inf test function for any numeric format."""
    sFormat = dictReport.get("sFormat", "")
    dictGenerators = {
        "npy": _fsNoNanNpy,
        "npz": _fsNoNanNpz,
        "csv": _fsNoNanCsv,
        "whitespace": _fsNoNanWhitespace,
        "fits": _fsNoNanFits,
        "matlab": _fsNoNanMatlab,
        "parquet": _fsNoNanParquet,
        "image": _fsNoNanImage,
        "vcf": _fsNoNanTabular,
        "bed": _fsNoNanTabular,
        "gff": _fsNoNanTabular,
        "sam": _fsNoNanTabular,
        "fortran": _fsNoNanFortran,
        "spss": _fsNoNanDataframe,
        "stata": _fsNoNanDataframe,
        "sas": _fsNoNanDataframe,
        "votable": _fsNoNanVotable,
        "ipac": _fsNoNanIpac,
        "vtk": _fsNoNanVtk,
        "cgns": _fsNoNanCgns,
        "safetensors": _fsNoNanSafetensors,
        "excel": _fsNoNanExcel,
        "rdata": _fsNoNanRdata,
        "hdf5": _fsNoNanHdf5,
    }
...
def _fsNoNanNpy(sSafe, sFile, dictReport):
    """Return no-NaN test code for a numpy .npy file."""
    return (
        f"\ndef test_{sSafe}_no_nan():\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    d = np.load(sPath, allow_pickle=False)\n"
        f"    assert not np.any(np.isnan(d))\n"
        f"    assert not np.any(np.isinf(d))\n\n"
    )


def _fsNoNanNpz(sSafe, sFile, dictReport):
    """Return no-NaN test code for a numpy .npz archive."""
    return (
        f"\ndef test_{sSafe}_no_nan():\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    a = np.load(sPath, allow_pickle=False)\n"
        f"    for sKey in a.files:\n"
        f"        if np.issubdtype(a[sKey].dtype, np.number):\n"
        f"            assert not np.any(np.isnan(a[sKey]))\n"
        f"            assert not np.any(np.isinf(a[sKey]))\n\n"
    )


def _fsNoNanHdf5(sSafe, sFile, dictReport):
    """Return no-NaN test code for an HDF5 file."""
    return (
        f"\ndef test_{sSafe}_no_nan():\n"
        f"    import h5py\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    with h5py.File(sPath, 'r') as fh:\n"
        f"        def fnCheck(name, obj):\n"
        f"            if isinstance(obj, h5py.Dataset):\n"
        f"                d = np.array(obj)\n"
        f"                if np.issubdtype(d.dtype, np.number):\n"
        f"                    assert not np.any(np.isnan(d))\n"
        f"                    assert not np.any(np.isinf(d))\n"
        f"        fh.visititems(fnCheck)\n\n"
    )


def _fsNoNanCsv(sSafe, sFile, dictReport):
    """Return no-NaN test code for a CSV file."""
    ...
    return (
        ...
        f"        rows = list(csv.DictReader(fh))\n"
        f"    for row in rows:\n"
        f"        for sVal in row.values():\n"
        f"            try:\n"
        f"                fVal = float(sVal)\n"
        f"                assert not np.isnan(fVal)\n"
        f"                assert not np.isinf(fVal)\n"
        f"            except ValueError:\n"
        f"                pass\n\n"
    )


def _fsNoNanWhitespace(sSafe, sFile, dictReport):
    """Return no-NaN test code for a whitespace-delimited file."""
    return (
        f"\ndef test_{sSafe}_no_nan():\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    with open(sPath, encoding='utf-8') as fh:\n"
        f"        for sLine in fh:\n"
        f"            s = sLine.strip()\n"
        f"            if not s or s.startswith('#'):\n"
        f"                continue\n"
        f"            for sToken in s.split():\n"
        f"                try:\n"
        f"                    fVal = float(sToken)\n"
        f"                    assert not np.isnan(fVal)\n"
        f"                    assert not np.isinf(fVal)\n"
...
def _fsNoNanFits(sSafe, sFile, dictReport):
    """Return no-NaN test code for a FITS file."""
    return (
        f"\ndef test_{sSafe}_no_nan():\n"
        f"    from astropy.io import fits\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    with fits.open(sPath) as hduList:\n"
        f"        for hdu in hduList:\n"
        f"            if hdu.data is None:\n"
        f"                continue\n"
        f"            d = np.array(hdu.data, dtype=float)\n"
        f"            assert not np.any(np.isnan(d))\n"
        f"            assert not np.any(np.isinf(d))\n\n"
    )


def _fsNoNanMatlab(sSafe, sFile, dictReport):
    """Return no-NaN test code for a MATLAB .mat file."""
    return (
        f"\ndef test_{sSafe}_no_nan():\n"
        f"    from scipy.io import loadmat\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    d = loadmat(sPath)\n"
        f"    for k in d:\n"
        f"        if k.startswith('__'):\n"
        f"            continue\n"
        f"        if np.issubdtype(d[k].dtype, np.number):\n"
        f"            assert not np.any(np.isnan(d[k]))\n"
        f"            assert not np.any(np.isinf(d[k]))\n\n"
    )


def _fsNoNanParquet(sSafe, sFile, dictReport):
    """Return no-NaN test code for a Parquet file."""
    return (
        f"\ndef test_{sSafe}_no_nan():\n"
        f"    import pyarrow.parquet as pq\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    table = pq.read_table(sPath)\n"
        f"    for col in table.column_names:\n"
        f"        try:\n"
        f"            d = table.column(col).to_numpy().astype(float)\n"
        f"            assert not np.any(np.isnan(d))\n"
        f"            assert not np.any(np.isinf(d))\n"
...
def _fsNoNanImage(sSafe, sFile, dictReport):
    """Return no-NaN test code for an image file."""
    return (
        f"\ndef test_{sSafe}_no_nan():\n"
        f"    from PIL import Image\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    d = np.array(Image.open(sPath), dtype=float)\n"
        f"    assert not np.any(np.isnan(d))\n"
        f"    assert not np.any(np.isinf(d))\n\n"
    )


def _fsNoNanTabular(sSafe, sFile, dictReport):
    """Return no-NaN test code for a tab-delimited text file."""
    return (
        f"\ndef test_{sSafe}_no_nan():\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    with open(sPath, encoding='utf-8') as fh:\n"
        f"        for sLine in fh:\n"
        f"            if sLine.startswith('#') or sLine.startswith('@'):\n"
        f"                continue\n"
        f"            for sToken in sLine.strip().split('\\t'):\n"
        f"                try:\n"
        f"                    fVal = float(sToken)\n"
        f"                    assert not np.isnan(fVal)\n"
        f"                    assert not np.isinf(fVal)\n"
...
def _fsNoNanFortran(sSafe, sFile, dictReport):
    """Return no-NaN test code for a FORTRAN binary file."""
    return (
        f"\ndef test_{sSafe}_no_nan():\n"
        f"    from scipy.io import FortranFile\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
...
def _fsNoNanVotable(sSafe, sFile, dictReport):
    """Return no-NaN test code for a VOTable file."""
    return (
        f"\ndef test_{sSafe}_no_nan():\n"
        f"    from astropy.io.votable import parse\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    table = parse(sPath).get_first_table().to_table()\n"
        f"    for col in table.colnames:\n"
        f"        try:\n"
        f"            d = np.array(table[col], dtype=float)\n"
        f"            assert not np.any(np.isnan(d))\n"
        f"            assert not np.any(np.isinf(d))\n"
        f"        except (ValueError, TypeError):\n"
        f"            pass\n\n"
...
def _fsNoNanIpac(sSafe, sFile, dictReport):
    """Return no-NaN test code for an IPAC table file."""
    return (
        f"\ndef test_{sSafe}_no_nan():\n"
        f"    from astropy.io import ascii\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
...
def _fsNoNanVtk(sSafe, sFile, dictReport):
    """Return no-NaN test code for a VTK file."""
    return (
        f"\ndef test_{sSafe}_no_nan():\n"
        f"    import pyvista\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    mesh = pyvista.read(sPath)\n"
        f"    for name in mesh.array_names:\n"
        f"        d = np.array(mesh[name], dtype=float)\n"
        f"        assert not np.any(np.isnan(d))\n"
        f"        assert not np.any(np.isinf(d))\n\n"
    )


def _fsNoNanCgns(sSafe, sFile, dictReport):
    """Return no-NaN test code for a CGNS file."""
    return (
        f"\ndef test_{sSafe}_no_nan():\n"
        f"    import h5py\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        ...
        f"        datasets = []\n"
        f"        fh.visititems(\n"
        f"            lambda n, o: datasets.append(n)\n"
        f"            if isinstance(o, h5py.Dataset) else None\n"
        f"        )\n"
        f"        for ds in datasets:\n"
        f"            d = np.array(fh[ds])\n"
        f"            if np.issubdtype(d.dtype, np.number):\n"
        f"                assert not np.any(np.isnan(d))\n"
        f"                assert not np.any(np.isinf(d))\n\n"
...
def _fsNoNanSafetensors(sSafe, sFile, dictReport):
    """Return no-NaN test code for a safetensors file."""
    return (
        f"\ndef test_{sSafe}_no_nan():\n"
        f"    from safetensors import safe_open\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    with safe_open(sPath, framework='numpy') as fh:\n"
        f"        for key in fh.keys():\n"
        f"            d = fh.get_tensor(key).astype(float)\n"
        f"            assert not np.any(np.isnan(d))\n"
        f"            assert not np.any(np.isinf(d))\n\n"
    )


def _fsNoNanExcel(sSafe, sFile, dictReport):
    """Return no-NaN test code for an Excel file."""
    return (
        f"\ndef test_{sSafe}_no_nan():\n"
        f"    import openpyxl\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    wb = openpyxl.load_workbook(sPath, read_only=True)\n"
        f"    for row in wb.active.iter_rows(min_row=2, values_only=True):\n"
        f"        for cell in row:\n"
        f"            if isinstance(cell, float):\n"
        f"                assert not np.isnan(cell)\n"
        f"                assert not np.isinf(cell)\n"
        f"    wb.close()\n\n"
    )


def _fsNoNanRdata(sSafe, sFile, dictReport):
    """Return no-NaN test code for an R data file."""
    return (
        f"\ndef test_{sSafe}_no_nan():\n"
        f"    import pyreadr\n"
        f"    sPath = os.path.join(_STEP_DIRECTORY, {sFile!r})\n"
        f"    dictFrames = pyreadr.read_r(sPath)\n"
        f"    for sKey, df in dictFrames.items():\n"
        f"        for sCol in df.select_dtypes(include='number').columns:\n"
        f"            assert not df[sCol].isna().any()\n\n"
    )
...
def _fnAddJsonKeysTest(listParts, sSafe, dictReport, listJsonKeys):
    """Append a JSON top-level keys test to listParts."""
    sFile = dictReport["sFileName"]
    listParts.append(
        f"\ndef test_{sSafe}_json_keys():\n"
        f"    import json\n"
        f"    import os\n"
        f"    sPath = os.path.join(\n"
        f"        os.path.dirname(os.path.dirname("
        ...
        f"        {sFile!r},\n    )\n"
        f"    with open(sPath, encoding='utf-8') as fh:\n"
        f"        dictData = json.load(fh)\n"
    )
    for sKey in listJsonKeys:
        listParts.append(
            f"    assert {sKey!r} in dictData\n"
        )
    listParts.append("\n")
...
def _fdictWriteAllDeterministicTests(
    ...
    dictResult = {}
    sIntegrityCode = _fsGenerateIntegrityCode(listdictReports)
    dictResult["dictIntegrity"] = _fdictWriteTestFile(
        connectionDocker, sContainerId,
        sIntegrityCode, fsIntegrityTestPath(sDirectory),
    )
    sQualitativeCode = _fsGenerateQualitativeCode(listdictReports)
    dictResult["dictQualitative"] = _fdictWriteTestFile(
        connectionDocker, sContainerId,
        sQualitativeCode, fsQualitativeTestPath(sDirectory),
    )
