...
clone can reconstruct staleness state (see Phase 2 of the
...
_MARKER_DIR = _WORKSPACE_ROOT / ".vaibify" / "test_markers"
_WORKFLOWS_DIR = _WORKSPACE_ROOT / ".vaibify" / "workflows"
...
def _fsStepDirRepoRel(sDir):
    """Return a step directory as a posix path relative to the workspace.

    Accepts container-absolute paths (``/workspace/step1``), host-
    substituted absolute paths (used in tests), or already-relative
    paths. Produces a normalized repo-relative posix string so that
    workflow entries and the live __file__-derived directory can be
    compared on equal footing.
    """
    if not sDir:
        return ""
    sNorm = os.path.normpath(sDir)
    sWorkspace = os.path.normpath(str(_WORKSPACE_ROOT))
    if sNorm == sWorkspace:
        return ""
    if sNorm.startswith(sWorkspace + os.sep):
        sNorm = sNorm[len(sWorkspace) + 1:]
    elif sNorm.startswith("/workspace/"):
        sNorm = sNorm[len("/workspace/"):]
...
def _fsRepoRelFromFile(sFile, sStepDirRel):
    ...
    if sFilePosix.startswith("/workspace/"):
        return sFilePosix[len("/workspace/"):]
