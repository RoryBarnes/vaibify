...
jobs:
  reproduce:
    ...
    steps:
      ...
      - name: Install Vaibify
        run: pip install vaibify

      - name: Build container image
        run: vaibify build

      - name: Run pipeline
        run: vaibify start --run-pipeline
{% if bUploadArtifacts %}
...
def fnWriteWorkflow(config, sOutputPath=None):
    ...
    sOutputPath : str or None
        ...
        .github/workflows/vaibify.yml in the current directory.
    """
    if sOutputPath is None:
        sOutputPath = ".github/workflows/vaibify.yml"
