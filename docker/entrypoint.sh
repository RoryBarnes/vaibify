...
**Never compute a quantitative or statistical result with a throwaway construction** — no heredocs, `python -c`, inline one-liners, or REPL sessions. A number that isn't produced by a saved script is not reproducible and cannot become a step. When asked to compute, estimate, fit, sample, or analyze anything numeric, write it as a script (`dataFoo.py` in the appropriate step directory, or the project's language) that takes its inputs as arguments and writes its outputs to files, then run it through `vaibify-do`. Even a quick exploratory answer is delivered as the script. See the `reproducible-analysis` skill for how to structure it so it can become a step.
