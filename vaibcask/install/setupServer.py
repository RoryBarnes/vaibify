...
from vaibcask.config.templateManager import flistAvailableTemplates
from vaibcask.config.projectConfig import fdictLoadDefaults
...
def fappCreateSetupWizard(sOutputDirectory="."):
    ...
    sOutputDirectory : str
        Directory where vaibcask.yml will be written.
    ...
    """
    app = FastAPI(title="VaibCask Setup Wizard")
...
def _fnRegisterWriteRoutes(app, sOutputDirectory):
    ...
    async def fnSaveConfig(request: ConfigRequest):
        ...
        sFilePath = str(
            Path(sOutputDirectory) / "vaibcask.yml"
        )
