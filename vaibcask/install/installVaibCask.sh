#!/bin/sh
# installVaibCask.sh - Install VaibCask and its host dependencies.
#
# Detects the operating system and package manager, installs Docker and
# the GitHub CLI, clones the VaibCask repository, creates symlinks
# so that "vaibcask" and "vc" are available on PATH, and adds the
# VaibCask bin directory to the user's shell configuration.
#
# Usage:
#   sh installVaibCask.sh [-y|--yes] [--claude]

set -e

VC_REPO="https://github.com/RoryBarnes/VaibCask.git"
    ...
    while [ $# -gt 0 ]; do
        case "$1" in
            ...
            *)
                ...
                echo "Usage: sh installVaibCask.sh [-y|--yes] [--claude]" >&2
                exit 1
                ;;
        esac
        shift
...
# ---------------------------------------------------------------------------
# fnCloneAndLink: Clone VaibCask and create symlinks
# ---------------------------------------------------------------------------
fnCloneAndLink() {
    local sBinDir="$1"

    if [ -x "./vaibcask" ] && [ -d "./docker" ]; then
        echo "[install] Already inside the VaibCask repository."
    elif [ -d "VaibCask" ]; then
        echo "[install] VaibCask directory already exists. Skipping clone."
        cd VaibCask
    else
        echo "[install] Cloning VaibCask..."
        git clone "${VC_REPO}"
        cd VaibCask
    fi

    echo "[install] Creating symlink at ${sBinDir}/vaibcask..."
    sudo ln -sf "$(pwd)/vaibcask/cli/main.py" "${sBinDir}/vaibcask"
    echo "[install] Creating symlink at ${sBinDir}/vc..."
    sudo ln -sf "$(pwd)/vaibcask/cli/main.py" "${sBinDir}/vc"
...
fnAppendToShellConfig() {
    ...
    {
        echo ""
        echo "# Added by VaibCask installer"
        echo "${sLine}"
    } >> "${sRcFile}"
}

# ---------------------------------------------------------------------------
# fnConfigureShellPath: Add the VaibCask bin directory to PATH
...
fnConfigureCompletions() {
    ...
    case "${sShellName}" in
        bash) sCompletionFile="${sVcDirectory}/completions/vaibcask.bash" ;;
        zsh)  sCompletionFile="${sVcDirectory}/completions/vaibcask.zsh" ;;
        *)    return ;;
    esac
    local sRcFile
    sRcFile=$(fsDetectShellRcFile "${sShellName}")
    if [ -f "${sRcFile}" ] && grep -qF "/VaibCask/completions/" "${sRcFile}" 2>/dev/null; then
...
# ---------------------------------------------------------------------------
# fnEnableClaude: Mark VaibCask to include Claude Code in Docker image
# ---------------------------------------------------------------------------
fnEnableClaude() {
    local sVcDirectory="$1"
...
if [ "${sPlatform}" = "Darwin" ]; then
    ...
    if command -v apt-get > /dev/null 2>&1; then
        ...
        echo "VaibCask requires apt (Debian/Ubuntu) or dnf (Fedora/RHEL)." >&2
        exit 1
    fi
    echo ""
    echo "[install] Installation complete."
...
if [ "${sPlatform}" = "Darwin" ]; then
    echo "[install] After starting Colima, run 'vaibcask init' to begin."
else
    echo "[install] After logging out and back in, run 'vaibcask init' to begin."
fi
